...
class Neighborhood {
  public:
    ...
    COMPLEX compute_value(COMPLEX const&, N_map&, std::vector<int>&);
    double update_value(COMPLEX const&, N_map&, std::vector<int>&);
...
COMPLEX Neighborhood::compute_value(COMPLEX const &z, N_map &N_diff, std::vector<int> &r) {

  int size = nodes.size();
  if (size==0) return self_loop;

  if (local_edges.size()==0) {
    COMPLEX ans = self_loop;
    for (int j=0; j<size; ++j) {
	ans += v[j]*u[j]/(z - N_diff.at((r[nodes[j]]==-1)?-1:f_node).at(nodes[j]).c_value);
    }
    return ans;
  }
    ...
    for (int j=0; j<size; ++j) {
      M(j,j) = (z - N_diff.at((r[nodes[j]]==-1)?-1:f_node).at(nodes[j]).c_value) ;
    }
...
double Neighborhood::update_value(COMPLEX const &z, N_map &N_diff, std::vector<int> &r) {
  auto new_value = compute_value(z,N_diff,r);
